...
export default function CampDaTaoPage() {
  ...
  async function fetchPosts(page = 1) {
    setLoading(true)
    const params = new URLSearchParams({ page: String(page), limit: String(PER_PAGE), status: "created" })
  ...
  async function backfillAccount() {
    ...
    if (!await ask("Backfill TKQC cho camp đã tạo trước đây (bị flag 'legacy' do bug cũ)?\n\nApp sẽ tự gán TKQC dựa trên Post của mỗi camp.\nKHÔNG ảnh hưởng camp trên FB.", { title: "Backfill TKQC" })) return
    setBackfilling(true)
    try {
      const r = await fetch("/api/campaigns/backfill-account", { method: "POST", credentials: "include" })
      const d = await r.json()
      if (!r.ok) throw new Error(d.error || `HTTP ${r.status}`)
      toast.show(d.message || `Đã gán ${d.fixed} camp`, "success" as any)
  ...
  async function bulkResetSelected() {
    ...
    if (!await ask(`Reset ${ids.length} post đã chọn về trạng thái chờ tạo lại?\n\n⚠ Camp cũ trên Facebook KHÔNG bị xoá — anh phải tự vào /quan-ly-campaign xoá nếu không muốn trùng tên.\n\nCron đầu giờ tới sẽ tạo camp mới với cùng tên cũ.`, { title: "Reset (tạo lại) loạt", warn: true })) return
    setBulkResetting(true)
    try {
      const r = await fetch("/api/posts/bulk-clear", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json", "x-confirm": "yes" },
        body: JSON.stringify({ mode: "reset-created", postIds: ids }),
      })
      const d = await r.json()
      if (!r.ok) throw new Error(d.error || `HTTP ${r.status}`)
  ...
  async function resetAll() {
    if (resetting) return
    if (!await ask(`Reset toàn bộ ${total} post 'Camp đã tạo' về trạng thái chờ tạo lại?\n\n⚠ Camp cũ trên Facebook KHÔNG bị xoá — sẽ trùng tên với camp mới tạo!\n\nCron đầu giờ tới sẽ tạo camp mới cùng tên với camp cũ.`, { title: "Reset toàn bộ", warn: true })) return
    setResetting(true)
    try {
      const r = await fetch("/api/posts/bulk-clear", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json", "x-confirm": "yes" },
        body: JSON.stringify({ mode: "reset-created" }),
      })
      const d = await r.json()
      if (!r.ok) throw new Error(d.error || `HTTP ${r.status}`)
  ...
  async function loadRestorePreview(hours: number) {
    ...
    try {
      const r = await fetch(`/api/posts/restore-deleted?hoursAgo=${hours}&mode=created`, { credentials: "include" })
      if (r.ok) {
        const d = await r.json()
        setRestorePreview({ count: d.count, sample: d.sample })
      }
    } catch {} finally { setRestoreLoading(false) }
  }
  async function doRestore() {
    if (restoring) return
    if (!restorePreview || restorePreview.count === 0) return
    if (!await ask(`Khôi phục ${restorePreview.count} post đã xoá nhầm (trong ${restoreHours} giờ)?\n\nPost sẽ về lại danh sách 'Camp đã tạo' đúng trạng thái cũ.`, { title: "Khôi phục post đã xoá" })) return
    setRestoring(true)
    try {
      const r = await fetch("/api/posts/restore-deleted", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ hoursAgo: restoreHours, mode: "created" }),
      })
      const d = await r.json()
      if (!r.ok) throw new Error(d.error || `HTTP ${r.status}`)
  ...
  return (
    <AppLayout>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" as const, gap: 8 }}>
        <div style={{ fontSize: 16, fontWeight: 600 }}>Camp đã tạo ({total})</div>
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={backfillAccount} disabled={backfilling} title="Gán lại TKQC cho camp bị flag 'legacy' (bug cũ chưa lưu adAccountId). Chỉ cần chạy 1 lần."
            style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 6, fontSize: 11, cursor: backfilling?"wait":"pointer", border: "1px solid rgba(79,126,248,.4)", fontFamily: "inherit", fontWeight: 500, background: "rgba(79,126,248,.08)", color: "var(--accent)", height: 30, whiteSpace: "nowrap" as const, opacity: backfilling?0.6:1 }}>
            {backfilling ? "⏳ Đang backfill..." : "🔧 Backfill TKQC camp legacy"}
          </button>
          ...
          {/* {isAdmin && (
            <button onClick={openRestore} title="Khôi phục post đã bị xoá nhầm (trong 24h gần đây)"
          ...
          {isAdmin && total > 0 && (
            <button onClick={resetAll} disabled={resetting} title="Reset toàn bộ post đã tạo về pending để tạo lại camp"
              ...
              {resetting ? "⏳..." : "🔄 Reset tạo lại tất cả"}
            </button>
          )}
        </div>
      </div>
        ...
        <div onClick={() => setShowRestore(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 10, padding: 18, width: 560, maxWidth: "100%", maxHeight: "84vh", overflowY: "auto" as const, display: "flex", flexDirection: "column" as const, gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ fontSize: 15, fontWeight: 600 }}>↩ Khôi phục post đã xoá</div>
            ...
            <div style={{ fontSize: 11, color: "var(--muted)" }}>
              Hệ thống dùng <strong>soft delete</strong> — post bị "Xoá hết data" KHÔNG mất, chỉ ẩn đi. Có thể khôi phục trong 24-168h gần đây.
            </div>
              ...
              <>
                <div style={{ padding: "10px 14px", background: restorePreview.count > 0 ? "rgba(46,204,143,.08)" : "rgba(120,120,120,.08)", borderRadius: 6, border: `1px solid ${restorePreview.count > 0 ? "rgba(46,204,143,.3)" : "var(--border2)"}` }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: restorePreview.count > 0 ? "var(--success)" : "var(--muted)" }}>
                    {restorePreview.count > 0 ? `✅ Tìm thấy ${restorePreview.count} post đã xoá` : "Không có post nào bị xoá trong khoảng này"}
                  </div>
                </div>
                    ...
                    {restorePreview.sample.map((p: any) => (
                      <div key={p.id} style={{ padding: "8px 10px", borderTop: "1px solid var(--border)", fontSize: 11 }}>
                        ...
                        <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 2 }}>
                          {p.campaign?.name && <span style={{ color: "var(--success)" }}>{p.campaign.name} • </span>}
      ...
      {(selectedIds.size > 0 || posts.length > 0) && (
        <div style={{ display: "flex", gap: 8, alignItems: "center", padding: "8px 12px", background: selectedIds.size > 0 ? "rgba(245,166,35,.08)" : "var(--bg2)", border: "1px solid " + (selectedIds.size > 0 ? "rgba(245,166,35,.4)" : "var(--border)"), borderRadius: 8, flexWrap: "wrap" as const }}>
          <span style={{ fontSize: 12, fontWeight: 500, color: selectedIds.size > 0 ? "var(--warn)" : "var(--muted)" }}>
            {selectedIds.size > 0 ? `✓ Đã chọn ${selectedIds.size} post` : `Tổng ${posts.length} post hiển thị`}
          </span>
          {posts.length > 0 && (
            <button onClick={toggleSelectAll} style={{ padding: "4px 10px", fontSize: 11, borderRadius: 5, cursor: "pointer", border: "1px solid var(--border)", background: "transparent", color: "var(--text)" }}>
              {posts.every(p => selectedIds.has(p.id)) ? "Bỏ chọn tất cả" : "Chọn tất cả (trang này)"}
            </button>
          )}
          {selectedIds.size > 0 && (
            <>
              ...
              <button onClick={bulkResetSelected} disabled={bulkResetting} style={{ padding: "4px 12px", fontSize: 11, borderRadius: 5, cursor: bulkResetting ? "wait" : "pointer", border: "none", background: "var(--warn)", color: "#fff", fontWeight: 600, opacity: bulkResetting ? 0.6 : 1 }}>
                {bulkResetting ? "⏳ Đang reset..." : `🔄 Tạo lại ${selectedIds.size} đã chọn`}
              </button>
            </>
          )}
        </div>
      ...
      <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed" as const, minWidth: 780 }}>
          <colgroup>
            <col style={{ width: 32 }}/><col style={{ width: 44 }}/><col /><col style={{ width: "16%" }}/><col style={{ width: "16%" }}/><col style={{ width: "14%" }}/><col style={{ width: 150 }}/>
          </colgroup>
          <thead>
            <tr style={{ background: "var(--bg3)" }}>
              <th style={{ padding: "8px 8px", textAlign: "center" as const, borderBottom: "1px solid var(--border)" }}>
                <input type="checkbox" checked={posts.length > 0 && posts.every(p => selectedIds.has(p.id))} onChange={toggleSelectAll} title="Chọn tất cả trên trang này" />
              </th>
              {["STT","BAI DANG","FANPAGE","TAI KHOAN","TEN CAMPAIGN","THOI GIAN TAO"].map(h => (
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={7} style={{ padding: 32, textAlign: "center", color: "var(--muted)" }}>Dang tai...</td></tr>
            ) : posts.length === 0 ? (
              <tr><td colSpan={7} style={{ padding: 32, textAlign: "center", color: "var(--muted)" }}>Khong co bai phu hop</td></tr>
            ) : posts.map((p, i) => (
              <tr key={p.id} style={{ borderBottom: "1px solid var(--border)", background: selectedIds.has(p.id) ? "rgba(245,166,35,.04)" : "transparent" }}>
                ...
                <td style={{ padding: "10px 12px", fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }} title={p.page?.name||""}>{p.page?.name||p.pageId||"—"}</td>
                <td style={{ padding: "10px 12px", fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }} title={(() => {
                  const acc = p.adAccount || p.page?.account || p.campaign?.adAccount
                  const src = p.adAccount ? "" : (p.page?.account ? " (legacy - từ page)" : (p.campaign?.adAccount ? " (legacy - từ camp)" : ""))
                  return acc ? `${acc.name} (${acc.actId})${src}` : ""
                })()}>
                  {(() => {
                    const acc = p.adAccount || p.page?.account || p.campaign?.adAccount
                    const isLegacy = !p.adAccount && !!acc
                    if (!acc) return <span style={{ color: "var(--muted)" }}>—</span>
                    return (
                      <div>
                        <div style={{ fontSize: 11, fontWeight: 500 }}>{acc.name}{isLegacy && <span style={{ fontSize: 9, color: "var(--muted)", marginLeft: 4 }}>(legacy)</span>}</div>
                        <div style={{ fontSize: 9, color: "var(--muted)", fontFamily: "monospace" }}>{acc.actId}</div>
                      </div>
                    )
                  })()}
                </td>
                <td style={{ padding: "10px 12px", fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const, color: "var(--success)" }}>{p.campaign?.name||"—"}</td>
                <td style={{ padding: "10px 12px", color: "var(--muted)", fontSize: 11, whiteSpace: "nowrap" as const }}>{p.adCreatedAt ? new Date(p.adCreatedAt).toLocaleString("vi-VN") : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
