...
const SMIT_ALARM = "fbads-smit-auto-sync"
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  console.log("[FBAds SMIT] Alarm setup random 20-50p")
})
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  console.log("[FBAds SMIT] Trigger luc " + new Date().toLocaleString("vi-VN"))
  try {
    await runSmitSync()
  } catch (e) {
    console.error("[FBAds SMIT] runSmitSync exception:", e?.message)
  }
  // Schedule next random run
  const nextMin = nextSmitDelayMin()
  await chrome.alarms.create(SMIT_ALARM, { delayInMinutes: nextMin })
  console.log("[FBAds SMIT] Next run in " + nextMin + " phut")
})
...
async function runSmitSync() {
  ...
  try {
    ...
    console.log(`[FBAds SMIT] Scraped ${rows.length} rows. Sample actIds:`, rows.slice(0, 3).map(r => r.actId))
    console.log(`[FBAds SMIT] Sample data:`, rows.slice(0, 2))
    if (rows.length === 0) {
    ...
    console.log(`[FBAds SMIT] Backend response:`, data)
    ...
    console.log("[FBAds SMIT] " + summary)
    return { ok: true, rowCount: rows.length, savedCount: data.saved || 0 }
  } catch (e) {
    const msg = String(e?.message || e)
    console.error("[FBAds SMIT] runSmitSync error:", msg)
    await chrome.storage.local.set({
      smitLastSyncAt: Date.now(),
      smitLastError: msg,
    })
